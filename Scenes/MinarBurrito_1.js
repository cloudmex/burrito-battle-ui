...
export class MinarBurrito extends Phaser.Scene{
    ...
    async Start(){
        ...
        if(remainToBuy == 0){
            ...
            new Helpers.Button(360,  this.sys.game.scale.gameSize.height + 2350, 0.25, "buttonContainer2", "Comprar", this, this.BuyTokens, null, {fontSize: 20, fontFamily: "BangersRegular"}, false)
        } else {
        ...
        if(localStorage.getItem("action") == "buyStraw"){
            ...
            if(info != null){
                ...
                this.GetTokens(parseInt(info.receipts_outcome[0].outcome.logs[0]));
            } else
                console.log("error");
        } else
            this.MintBurrito();
    ...
    async GetTokens (tokens = null) {
        if(tokens == null)  
            tokens = await Near.BuyTokens();
    ...
    GetBurrito = async () => {
        ...
        //let value = JSON.parse('{"attack":"8","burrito_type":"Volador","defense":"7","description":"Este es un burrito de tipo Volador","global_win":"0","hp":"5","level":"1","media":"QmQcTRnmdFhWa1j47JZAxr5CT1Cdr5AfqdhnrGpSdr28t6","name":"Burrito Volador #81","owner_id":"jesusrobles.testnet","speed":"5","win":"0"}')
        this.MintBurrito(value);
    }
    ...
    async MintBurrito(minar = null){
        minar = {attack:5,burrito_type:"Fuego",defense:6 ,description:"Este es un burrito de tipo Fuego", global_win:"0", hp:"5", level:"1", media:"QmZEK32JEbJH3rQtXL9BqQJa2omXfpjuXGjbFXLiV2Ge9D", name:"Burrito Fuego #22",owner_id:"jesus13th.testnet",speed:3,win:"0"}
        if(minar == null)
            minar = await Near.GetState();
        localStorage.removeItem("action");
