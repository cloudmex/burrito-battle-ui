...
export class Hospital extends Phaser.Scene{
    ...
    async loadAssets(){
        ...
        this.load.spritesheet("capsulas", "./src/images/Hospital/capsulas.png", {frameWidth:239, frameHeight:294});
        this.load.image("hospital_background", "./src/images/Hospital/Interior Hospital.png");
        this.load.image("hospital_NPC", "./src/images/Burrito Agua.png")
        ...
        this.load.image("dialogue_box", "../src/images/dialogue-box.png");
        this.load.image("dialog", "../src/images/Dialog.png");
    ...
    async start(){
        ...
        new Helpers.Button(this.game.config.width / 2,  60, 0.5, "buttonContainer", Translate.Translate("BtnBackToMeadow"), this, () => {this.scene.start("newMap")}, null, {fontSize: 24, fontFamily: "BangersRegular"});
        ...
        this.input.on("pointerdown", function(pointer){
            if(!this.burrito.anims.isPlaying && !Helpers?.Alert.isAlert && !this.isPanel){
        ...
        //new Helpers.DialogueBox(this, 700, 500, text, true, .8, ()=>{this.isPanel = false;});
    }
    ShowAlert = async(title, description, scene) => {
        if(!this.showAlert){
            ...
            this.burrito.stop();
            await Helpers.Alert.Fire(this, this.game.config.width / 2, this.game.config.height / 2, title, description, "Ir a pradera", "Cancelar")
            .then(async (result) => {
    ...
    update(){
        if(this.isPanel || Helpers.Alert.isAlert){
            return;
        }
    ...
    async UseCapsule(capsule){
            ...
            if(selectedCapsule.burrito_id != "" && now > finish){            
                ...
                await this.loadingScreen?.OnComplete();
                new Helpers.DialogueBox(this, 700, 500, ["¡Tu burrito está listo!, ya volverá a tu establo para que puedas volver a usarlo"], true, 0.8, ()=>{this.isPanel = false;});
                ...
                new Helpers.DialogueBox(this, 700, 500, text, true, 0.8, ()=>{this.isPanel = false;});
                return;
            ...
            if( await Near.GetSTRWToken() >= this.requiredSTRW){
                ...
                new Helpers.DialogueBox(this, 700, 500, ["No tienes suficientes $STRW para ingresar a tu burrito"], true, 0.8, ()=>{this.isPanel = false;});
            }
    }
    async CreatePanel(capsule){
        ...
        this.panelContainer.add(new Helpers.Button(600, -500, 0.25, "cerrar", null, this, ()=>{ this.panelContainer.destroy(); this.isPanel = false; }, null, null).GetComponents());
    ...
    SpawnCards = async(capsule) => {
        ...
        burritos.forEach((burrito, index) => {
            let card = new Helpers.Card( (300 * (index % 3)) - 300, (380 * Math.floor(index / 3)) - 125, burrito, this, true, false, false, false).setScale(0.35).On(() => { this.EnterBurrito(burrito, capsule)});
    ...
    async EnterBurrito(burrito, capsule){
        ...
        this.isPanel = false; 
        await this.loadingScreen?.OnComplete();
        new Helpers.DialogueBox(this, 700, 500, ["Listo, ahora debes esperar una época para volver por tu burrito"], true, 0.8, ()=>{this.isPanel = true;});
